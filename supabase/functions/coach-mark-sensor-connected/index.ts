/**
 * Coach Mark Sensor Connected API
 * ================================
 * Mark a player as connected to DK group after successful invite.
 * 
 * POST /coach-mark-sensor-connected
...
interface MarkConnectedRequest {
  phone: string;
}
...
serve(async (req) => {
  ...
  let requestBody: MarkConnectedRequest | null = null;
  ...
  try {
    ...
    if (!expectedKey || token !== expectedKey) {
      console.log("[MarkSensorConnected] Invalid API key attempt from:", clientIP);
      responseStatus = 401;
      return new Response(
    ...
    // Parse request body
    requestBody = await req.json() as MarkConnectedRequest;
    ...
    console.log(`[MarkSensorConnected] Marking sensor connected for phone: ****${last10.slice(-4)}`);
    ...
    if (playerError) {
      console.error("[MarkSensorConnected] Player lookup error:", playerError);
      responseStatus = 500;
      return new Response(
    ...
    if (player.sensor_connected) {
      ...
      return new Response(
        JSON.stringify({
          ...
          sensor_connected_at: "already_connected",
          message: "Player was already marked as connected"
        }),
    ...
    const { error: updateError } = await supabase
      ...
      .update({
        sensor_connected: true,
        sensor_connected_at: connectedAt,
        updated_at: connectedAt,
      })
      .eq("id", player.id);

    if (updateError) {
      console.error("[MarkSensorConnected] Update error:", updateError);
      responseStatus = 500;
      return new Response(
    ...
    console.log(`[MarkSensorConnected] Successfully marked player ${player.id} as sensor connected`);
    ...
    console.error("[MarkSensorConnected] Error:", error);
    ...
    try {
      ...
      console.error("[MarkSensorConnected] Audit log error:", auditError);
    }
