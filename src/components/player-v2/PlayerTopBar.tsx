...
export function PlayerTopBar({ playerName, motorProfile }: PlayerTopBarProps) {
  ...
  return (
    <header className="sticky top-0 z-50 px-4 py-3 flex items-center justify-between" style={{ background: '#0a0a0a', borderBottom: '1px solid #222' }}>
      <div>
        <p className="text-xs" style={{ color: '#777' }}>{getGreeting()}</p>
        <p className="text-lg font-bold" style={{ color: '#fff', fontFamily: "'DM Sans', sans-serif" }}>{firstName}</p>
      </div>
